...
5. Запустите приложение: `java -jar target/training-diary-1.1.jar`
...
- В проекте реализована с базой данных с использованием JDBC. 
- Созданы классы репозиториев, такие как UserRepositoryJDBC и TrainingRepositoryJDBC, для выполнения операций CRUD с базой данных. 
- Используется объект Connection для подключения к базе данных, передаваемый в конструктор репозиториев. 
- Добавлены методы для выполнения транзакций и обработки исключений для обеспечения целостности данных.
- Проект покрыт тестами, написанными с использованием `JUnit`, `Mockito` и `AsserJ` и `TestContainers`. 
